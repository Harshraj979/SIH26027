...
export default function TMSPortalView({
  ...
  const averageTqi = tmsOrders.length
    ? tmsOrders.reduce((sum, w) => sum + (Number.isFinite(w.tqiScore) ? w.tqiScore : 0), 0) / tmsOrders.length
    : null;
  const stagedRakes = new Set(tmsOrders.map((w) => w.ssrTaskCode).filter(Boolean)).size;

  const [selectedSSR, setSelectedSSR] = useState(TMS_SSR_OPTIONS[0]);
  const [selectedSection, setSelectedSection] = useState(SECTIONS[0]);
  const [lineId, setLineId] = useState("UP_FAST");
  const [kmFrom, setKmFrom] = useState("14.2");
  const [kmTo, setKmTo] = useState("14.3");
  const [railwayLocation, setRailwayLocation] = useState("UP line, cess side");
  const [assetType, setAssetType] = useState("RAIL");
  const [assetId, setAssetId] = useState("RAIL-UP-0142");
  const [inspectionMethod, setInspectionMethod] = useState("Visual inspection");
  const [protectionStatus, setProtectionStatus] = useState("REQUIRED");
  const [defectSeverity, setDefectSeverity] = useState<"CRITICAL" | "MAJOR" | "MINOR">("CRITICAL");
  ...
  const kmNum = parseFloat(kmFrom);
  const kmToNum = parseFloat(kmTo);
  const depotKm = !Number.isFinite(kmNum) ? 0 : kmNum <= 45 ? 0.0 : kmNum <= 140 ? 90.0 : kmNum <= 250 ? 197.0 : 312.0;
  const depotName = !Number.isFinite(kmNum) ? "Enter a valid KM range" : kmNum <= 45 ? "New Delhi Yard Depot (KM 0.0)" :
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!Number.isFinite(kmNum) || !Number.isFinite(kmToNum) || kmToNum <= kmNum ||
        kmNum < selectedSection.kmStart || kmToNum > selectedSection.kmEnd ||
        !Number.isFinite(Number(tqiScore)) || Number(tqiScore) < 0 || Number(tqiScore) > 100) {
      setSubmittedMessage("Correct the highlighted numeric values: KM range must be inside the selected section and TQI must be 0–100.");
      return;
    }
    ...
    onAddWorkOrder({
      ...
      defectSeverity,
      inspectionMethod,
      protectionStatus: protectionStatus as "REQUIRED" | "IN_PLACE" | "NOT_REQUIRED",
      ...
      kpMarker: `Section: ${selectedSection.id} | Line: ${lineId} | KM ${kmNum.toFixed(1)}–${kmToNum.toFixed(1)}`,
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-2xs">
          ...
          <p className="text-2xl font-black text-slate-800 mt-1">{stagedRakes}</p>
          <p className="text-[11px] text-slate-500 mt-0.5">Distinct TMS task types logged</p>
        </div>
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-2xs">
          <p className="text-[10.5px] uppercase font-bold text-slate-500">Average Section TQI</p>
          <p className="text-2xl font-black text-emerald-700 mt-1">{averageTqi === null ? "—" : averageTqi.toFixed(1)}</p>
          <p className="text-[11px] text-slate-500 font-medium mt-0.5">Average from TMS work orders</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-800 mb-2">
              1. Select Engineering Maintenance Task (Demo TMS Master Catalogue)
            </label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2 border-t border-slate-100">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-800 mb-1">
                Railway location and asset
              </label>
              <div className="flex gap-2">
                <input type="text" value={railwayLocation} onChange={(e) => setRailwayLocation(e.target.value)}
                  placeholder="Location (e.g. UP line, cess side)" className="w-1/2 text-xs bg-slate-50 border border-slate-300 rounded-lg p-2.5 text-slate-800" required />
                <select value={assetType} onChange={(e) => setAssetType(e.target.value)}
                  className="w-1/2 text-xs bg-slate-50 border border-slate-300 rounded-lg p-2.5 text-slate-800">
                  <option value="RAIL">Rail</option><option value="TURNOUT">Turnout</option>
                  <option value="SLEEPER">Sleeper</option><option value="BALLAST">Ballast</option>
                  <option value="DRAIN">Drain / cess</option>
                </select>
              </div>
              <input type="text" value={assetId} onChange={(e) => setAssetId(e.target.value)}
                placeholder="Asset ID / register reference" className="w-full mt-2 text-xs bg-slate-50 border border-slate-300 rounded-lg p-2.5 font-mono text-slate-800" required />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div><label className="block text-xs font-bold text-slate-800 mb-1">KM From (numeric)</label>
                <input type="number" step="0.001" min={selectedSection.kmStart} max={selectedSection.kmEnd} value={kmFrom} onChange={(e) => setKmFrom(e.target.value)} className="w-full text-xs bg-slate-50 border border-slate-300 rounded-lg p-2.5 font-mono" required /></div>
              <div><label className="block text-xs font-bold text-slate-800 mb-1">KM To (numeric)</label>
                <input type="number" step="0.001" min={selectedSection.kmStart} max={selectedSection.kmEnd} value={kmTo} onChange={(e) => setKmTo(e.target.value)} className="w-full text-xs bg-slate-50 border border-slate-300 rounded-lg p-2.5 font-mono" required /></div>
              <div><label className="block text-xs font-bold text-slate-800 mb-1">TQI (0–100)</label>
                <input type="number" step="0.1" min="0" max="100" value={tqiScore} onChange={(e) => setTqiScore(e.target.value)} className="w-full text-xs bg-slate-50 border border-slate-300 rounded-lg p-2.5 font-mono" required /></div>
              <div><label className="block text-xs font-bold text-slate-800 mb-1">Defect severity</label>
                <select value={defectSeverity} onChange={(e) => setDefectSeverity(e.target.value as typeof defectSeverity)} className="w-full text-xs bg-slate-50 border border-slate-300 rounded-lg p-2.5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div><label className="block text-xs font-bold text-slate-800 mb-1">Inspection method</label>
              <select value={inspectionMethod} onChange={(e) => setInspectionMethod(e.target.value)} className="w-full text-xs bg-slate-50 border border-slate-300 rounded-lg p-2.5">
                <option>Visual inspection</option><option>USFD</option><option>Track geometry recording</option><option>Joint inspection</option>
              </select></div>
            <div><label className="block text-xs font-bold text-slate-800 mb-1">Protection status</label>
              <select value={protectionStatus} onChange={(e) => setProtectionStatus(e.target.value)} className="w-full text-xs bg-slate-50 border border-slate-300 rounded-lg p-2.5">
                <option value="REQUIRED">Required before work</option><option value="IN_PLACE">In place</option><option value="NOT_REQUIRED">Not required</option>
              </select></div>
          </div>
