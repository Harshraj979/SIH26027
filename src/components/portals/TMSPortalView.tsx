    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-5 text-slate-800">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5">
        <div className="bg-white p-4 rounded-xl border border-slate-200/90 shadow-2xs">
          <p className="text-[10px] uppercase font-bold text-slate-500 tracking-wider">Active TMS Demands</p>
          <p className="text-2xl font-black text-amber-900 mt-0.5">{tmsOrders.length}</p>
          <p className="text-[11px] text-slate-500 font-medium">Civil P-Way Work Orders</p>
        </div>
        <div className="bg-white p-4 rounded-xl border border-slate-200/90 shadow-2xs">
          <p className="text-[10px] uppercase font-bold text-slate-500 tracking-wider">Critical Fractures</p>
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200/90 shadow-2xs">
          <p className="text-[10px] uppercase font-bold text-slate-500 tracking-wider">Track Machines Staged</p>
          <p className="text-2xl font-black text-slate-800 mt-0.5">4 Rakes</p>
          <p className="text-[11px] text-slate-500 font-medium">CSM-09 Tamping &amp; BCM-12</p>
        </div>
        <div className="bg-white p-4 rounded-xl border border-slate-200/90 shadow-2xs">
          <p className="text-[10px] uppercase font-bold text-slate-500 tracking-wider">Average Corridor TQI</p>
          <p className="text-2xl font-black text-emerald-700 mt-0.5">68.4</p>
          <p className="text-[11px] text-emerald-700 font-medium">Track Quality Index (Safe)</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-5 sm:p-6">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            {/* ── LEFT COLUMN: SSR Task Catalog (5 Cols) ───────────────────── */}
            <div className="lg:col-span-5 flex flex-col gap-3">
              ...
              <div className="p-3 bg-amber-50/50 rounded-xl border border-amber-200/80 text-xs space-y-1.5">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-black uppercase text-amber-900">Task Scope Spec:</span>
                  <span className="text-[10px] font-mono font-bold text-amber-800">{selectedSSR.code}</span>
                </div>
                <p className="text-[11px] text-slate-700 font-medium leading-relaxed">{selectedSSR.desc}</p>
                <div className="pt-1.5 border-t border-amber-200/60 flex items-center justify-between text-[10.5px] text-slate-600 font-semibold">
              ...
              <div className="p-3.5 rounded-xl bg-gradient-to-r from-slate-50 via-amber-50/30 to-amber-50/60 border border-amber-200/90 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                ...
                <div className="flex items-center justify-between sm:justify-end gap-3 shrink-0">
                  <div className="text-right">
                    ...
                    <p className={`text-base font-black ${previewRisk >= 90 ? "text-rose-700" : previewRisk >= 60 ? "text-amber-700" : "text-emerald-700"}`}>
      ...
      {/* ── 4. Active TMS Work Orders Table ─────────────────────────────────── */}
      <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
              ...
              {tmsOrders.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-8 text-center text-slate-400 text-xs">
                    No active Civil P-Way demands registered. Log a requisition above.
                  </td>
                </tr>
              ) : (
                tmsOrders.slice(0, 15).map((wo) => {
                  const isCriticalItem = (wo.assetRisk ?? 0) >= 90;
                  return (
